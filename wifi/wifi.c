...
static const char WLAN_SYS_FILE[]  ="/sys/android_wlan/wlan_power/sleep_policy";
...
int wifi_load_driver()
{
    ...
    int count_stat = 200; /*wait at most 20 seconds for wlan driver probe*/
    int status = -1;
    struct stat info;
    int ret = -1;
 ...
 /*           LOGD("ATH6KL getting executed");
	    while(count_stat-- > 0){
		if((ret = stat(WLAN_SYS_FILE, &info)) != 0){
			usleep(100000);
		} else{
...
static int _wifi_unload_driver()
{
    ...
    if (rmmod(DRIVER_MODULE_NAME) == 0) {
        ...
        if (count) {
		    ...
		    LOGE("failed to rmmod cfg80211.ko \n");
                    return -1;
                }
            }
		...
		LOGE("rmmod cannot complete \n");
		return -1;
	}
    }
    else{
	LOGE("failed to rmmod wlan.ko \n");
        return -1;
	}
}
